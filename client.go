// Package liqui is the unofficial client to access to Liqui API
package liqui

import (
	"bytes"
	"crypto/hmac"
	"crypto/sha512"
	"encoding/hex"
	"encoding/json"
	"fmt"
	"io/ioutil"
	"net/http"
	"net/url"
	"path"
	"strconv"
	"time"

	"golang.org/x/net/context"
)

const (
	// PublicAPIEndpoint is the Liqui public API endpoint.
	PublicAPIEndpoint = "https://api.liqui.io/api/3/"

	// TradeAPIEndpoint is the Liqui trade API endpoint.
	TradeAPIEndpoint = "https://api.liqui.io"
)

// Client manages all the communication with the Liqui API.
type Client struct {
	// Base URL for public API requests.
	BaseURL *url.URL

	// Base URL for trade API requests.
	BaseTradeURL *url.URL

	// Auth data
	APIKey    string
	APISecret string
}

// NewClient creates new Liqui API client.
func NewClient() *Client {
	baseURL, _ := url.Parse(PublicAPIEndpoint)
	baseTradeURL, _ := url.Parse(TradeAPIEndpoint)
	return &Client{BaseURL: baseURL, BaseTradeURL: baseTradeURL}
}

// NewRequest create new API request. Relative url can be provided in refURL.
func (c *Client) newRequest(ctx context.Context, method string, refURL string, params url.Values) (*http.Request, error) {
	rel, err := url.Parse(refURL)
	if err != nil {
		return nil, err
	}
	if params != nil {
		rel.RawQuery = params.Encode()
	}
	var req *http.Request
	u := c.BaseURL.ResolveReference(rel)
	req, err = http.NewRequest(method, u.String(), nil)

	if err != nil {
		return nil, err
	}

	req = req.WithContext(ctx)

	return req, nil
}

// newAuthenticatedRequest creates new http request for authenticated routes.
func (c *Client) newAuthenticatedRequest(ctx context.Context, method string, values url.Values) (*http.Request, error) {
	if values == nil {
		values = url.Values{}
	}

	values.Set("method", method)
	values.Set("nonce", strconv.FormatInt(time.Now().Unix(), 10))
	body := values.Encode()

	u := *c.BaseTradeURL
	u.Path = path.Join(c.BaseTradeURL.Path, "tapi")
	req, err := http.NewRequest("POST", u.String(), bytes.NewBufferString(body))
	if err != nil {
		return nil, err
	}
	req.Header.Add("Accept", "application/json")
	req.Header.Add("Content-Type", "application/x-www-form-urlencoded")

	// Sign request
	h := hmac.New(sha512.New, []byte(c.APISecret))
	h.Write([]byte(body))
	sign := hex.EncodeToString(h.Sum(nil))
	req.Header.Add("Key", c.APIKey)
	req.Header.Add("Sign", sign)

	req = req.WithContext(ctx)

	return req, nil
}

func (c *Client) signPayload(payload string) string {
	sig := hmac.New(sha512.New384, []byte(c.APISecret))
	sig.Write([]byte(payload))
	return hex.EncodeToString(sig.Sum(nil))
}

// Auth sets api key and secret for usage is requests that requires authentication.
func (c *Client) Auth(key string, secret string) *Client {
	c.APIKey = key
	c.APISecret = secret

	return c
}

var httpDo = func(req *http.Request) (*http.Response, error) {
	return http.DefaultClient.Do(req)
}

// Do executes API request created by NewRequest method or custom *http.Request.
func (c *Client) do(req *http.Request, v interface{}) (*Response, error) {
	resp, err := httpDo(req)

	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()

	response := newResponse(resp)

	err = checkResponse(response)
	if err != nil {
		// Return response in case caller need to debug it.
		return response, err
	}

	if v == nil {
		return response, nil
	}

	data := response.Body

	if req.URL.Path == "/tapi" {
		tRes := TradeAPIResponse{}
		err = json.Unmarshal(response.Body, &tRes)
		if err != nil {
			return response, err
		}
		if tRes.Success == 1 {
			data = tRes.Return
		}
	}

	err = json.Unmarshal(data, v)
	if err != nil {
		return response, err
	}

	return response, nil
}

// TradeAPIResponse is the trade API's response json format.
type TradeAPIResponse struct {
	Success int
	Return  json.RawMessage
}

// Response is wrapper for standard http.Response and provides
// more methods.
type Response struct {
	Response *http.Response
	Body     []byte
}

// newResponse creates new wrapper.
func newResponse(r *http.Response) *Response {
	body, err := ioutil.ReadAll(r.Body)
	if err != nil {
		body = []byte(`Error reading body:` + err.Error())
	}

	return &Response{r, body}
}

// String converts response body to string.
// An empty string will be returned if error.
func (r *Response) String() string {
	return string(r.Body)
}

// ErrorResponse is the custom error type that is returned if the API returns an
// error.
type ErrorResponse struct {
	Response *Response
	Message  string `json:"error"`
}

func (r *ErrorResponse) Error() string {
	return fmt.Sprintf("%v %v: %d %v",
		r.Response.Response.Request.Method,
		r.Response.Response.Request.URL,
		r.Response.Response.StatusCode,
		r.Message,
	)
}

// checkResponse checks response status code and response
// for errors.
func checkResponse(r *Response) error {
	if c := r.Response.StatusCode; 200 <= c && c <= 299 {
		// Liqui API returns 200 with error, so check here...
		res := map[string]interface{}{}
		json.Unmarshal(r.Body, &res)
		if val, ok := res["error"]; ok {
			return &ErrorResponse{Response: r, Message: "Error: " + val.(string)}
		}
		return nil
	}

	// Try to decode error message
	errorResponse := &ErrorResponse{Response: r}
	err := json.Unmarshal(r.Body, errorResponse)
	if err != nil {
		errorResponse.Message = "Error decoding response error message. " +
			"Please see response body for more information."
	}

	return errorResponse
}
